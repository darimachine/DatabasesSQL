...
SELECT NAME
FROM MOVIESTAR
WHERE GENDER ='m' AND BIRTHDATE=(SELECT MAX(BIRTHDATE) as b
FROM MOVIESTAR WHERE GENDER = 'm');
...
FROM STARSIN JOIN MOVIE as mv ON STARSIN.MOVIETITLE = mv.TITLE
WHERE STARSIN.STARNAME = (SELECT TOP 1 STARNAME
							FROM STARSIN as innerStarsin JOIN MOVIE as innerMv ON
							innerStarsin.MOVIETITLE = innerMv.TITLE
							AND mv.STUDIONAME = innerMv.STUDIONAME
							GROUP BY STARNAME
